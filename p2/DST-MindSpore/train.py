...
# python train.py --gpu 0 --data_path ../../../completion_blocks  --train_file_list ../filelists/train_list.txt --val_file_list ../filelists/val_list.txt --save logs/mp-test
# python train.py --gpu 1 --data_path ../../../completion_blocks  --train_file_list ../filelists/train_list.txt --val_file_list ../filelists/val_list.txt --save logs/mp-test
# params
parser = argparse.ArgumentParser()
# data paths
parser.add_argument('--gpu', type=int, default=5, help='which gpu to use')
parser.add_argument('--data_path', default='../../../../completion_blocks',required=False, help='path to data')
parser.add_argument('--train_file_list',default='../filelists/train_list.txt', required=False, help='path to file list of train data')
parser.add_argument('--val_file_list', default='../filelists/val_list.txt', help='path to file list of val data')
parser.add_argument('--save', default='logs/catn2', help='folder to output model checkpoints')
# parser.add_argument('--save', default='logs/stage1-connect2layer-transformer-nomask1-6-kvrec-821x8-normal3D2-ioumean-888-withoutcross', help='folder to output model checkpoints')
# model params
...
parser.add_argument('--vis_dfs', type=int, default=1, help='use df (iso 1) to visualize')
...
#args.gpu=1
#parser.set_defaults(data_path='../../../completion_blocks')
#args.data_path='../../../completion_blocks'
#args.train_file_list='../filelists/train_list.txt'
#args.val_file_list='../filelists/val_list.txt'
#args.save='logs/mp-test'
...
if args.retrain:
    ...
    checkpoint = mindspore.load(args.retrain)
    args.start_epoch = args.start_epoch if args.start_epoch != 0 else checkpoint['epoch']
    model.load_state_dict(checkpoint['state_dict']) #, strict=False)
    optimizer.load_state_dict(checkpoint['optimizer'])

def getLRScheduler(lr, step, gamma, last_epoch, max_epoch):
    k = (last_epoch + step) // step # ceil
    milestone = list(range(1 + k * step - last_epoch, max_epoch - last_epoch + 1, step))
    if (max_epoch - last_epoch) not in milestone:
        milestone = milestone + [max_epoch - last_epoch]
    learning_rates = [lr * (gamma ** (i+k-1)) for i in range(0, len(milestone))]
    return nn.piecewise_constant_lr(milestone, learning_rates)

last_epoch = -1 if not args.retrain else args.start_epoch - 1
# scheduler = getLRScheduler(lr=args.lr, step=args.decay_lr, gamma=0.5, last_epoch=last_epoch, max_epoch=args.max_epoch)
scheduler = nn.StepLR(optimizer, step_size=args.decay_lr, gamma=0.5)

# data files-
train_files, val_files = data_util.get_train_files(args.data_path, args.train_file_list, args.val_file_list, last_epoch=last_epoch)
_OVERFIT = False
...
train_dataloader = mindspore.dataset.GeneratorDataset(train_dataset, shuffle=True, num_parallel_workers=2)
train_dataloader = train_dataloader.batch(args.batch_size, per_batch_map=scene_dataloader.collate)
...
if len(val_files) > 0:
    ...
    val_dataloader = mindspore.dataset.GeneratorDataset(val_dataset, shuffle=False, num_parallel_workers=2)
    val_dataloader = val_dataloader.batch(args.batch_size, per_batch_map=scene_dataloader.collate)
...
def train(epoch, iter, dataloader, log_file, output_save):
    ...
    for t, sample in enumerate(dataloader):
        ...
        #print(inputs[0].shape,inputs[1].shape,inputs[0],inputs[1]) [200000,4] [200000,1] xyzb int
        time_readdata=time.time()
        ...
        time_compute_targets=time.time()
        #print('time_compute_targets', time_compute_targets-time_readdata )
        optimizer.zero_grad()
        output_sdf, output_occs = model(inputs, loss_weights)

        time_model=time.time()
        loss, losses,weights = loss_util.compute_loss(output_sdf, output_occs, target_for_sdf, target_for_occs, target_for_hier, loss_weights, args.truncation, args.logweight_target_sdf, args.weight_missing_geo, inputs[0], args.use_loss_masking, known)
        loss_normal,loss_normal_cos=loss_util.compute_normal_loss(output_sdf,target_for_sdf,args.input_dim,args.use_loss_masking, known,loss_weights,weights[-1])
        #loss_iou,loss_iou2 = loss_util.compute_iou_loss(output_sdf, target_for_sdf, args.input_dim, args.use_loss_masking, known,loss_weights, weights[-1])
        #print('loss_iou',loss_iou,loss_iou2)
        time_loss=time.time()
        #print('time_loss',time_loss- time_model )
        loss=loss+loss_normal+loss_normal_cos#+loss_iou2
        #print('loss',loss)
        loss.backward()
        time_lossback=time.time()

        #print('time_lossback', time_lossback-time_loss )
        optimizer.step()
        time_optimizer=time.time()
        #print('time_optimizer', time_optimizer-time_lossback )
        ...
        if compute_pred_occs:
            ...
            for h in range(args.num_hierarchy_levels):
                ...
                if len(output_occs[h][0]) == 0:
                    continue
                output_occs[h][1] = nn.Sigmoid()(output_occs[h][1][:,0]) > 0.4
        ...
        if len(output_sdf[0]) > 0:
            output_sdf = [output_sdf[0], output_sdf[1]]
        ...
        if iter % 2000 == 0:
            ...
            mindspore.save_checkpoint(model, os.path.join(args.save, 'model-iter%s-epoch%s-net.ckpt' % (iter, epoch)))
            mindspore.save_checkpoint(optimizer, os.path.join(args.save, 'model-iter%s-epoch%s-opt.ckpt' % (iter, epoch)))
        if output_visual:
            ...
            if len(output_sdf[0]) > 0:
                for b in range(args.batch_size):
                    ...
                    if len(mask) > 0:
                        vis_pred_sdf[b] = [output_sdf[0][mask].numpy(), output_sdf[1][mask].squeeze().numpy()]
            inputs = [inputs[0].numpy(), inputs[1].numpy()]
            for h in range(args.num_hierarchy_levels):
                for b in range(args.batch_size):
                    if pred_occs[h][b] is not None:
                        pred_occs[h][b] = pred_occs[h][b].numpy()
            #print('inputs',inputs)
            #print('vis_pred_sdf',vis_pred_sdf)
            data_util.save_predictions(os.path.join(args.save, 'iter%d-epoch%d' % (iter, epoch), 'train'), sample['name'], inputs, target_for_sdf.numpy(), [x.numpy() for x in target_for_occs], vis_pred_sdf, pred_occs, sample['world2grid'].numpy(), args.truncation,args.vis_dfs)
        time_later=time.time()
        #print('time_later',time_later- time_optimizer )
        ...
        if compute_pred_occs:
            ...
            for h in range(args.num_hierarchy_levels):
                ...
                for b in range(args.batch_size):
                    batchmask = output_occs[h][0][:,-1] == b
                    locs = output_occs[h][0][batchmask][:,:-1]
                    vals = nn.Sigmoid()(output_occs[h][1][:,0][batchmask]) > 0.5
                    pred_occs[h][b] = locs[vals.view(-1)]
        ...
        if output_visual:
            ...
            for h in range(args.num_hierarchy_levels):
                for b in range(args.batch_size):
                    if pred_occs[h][b] is not None:
                        pred_occs[h][b] = pred_occs[h][b].numpy()
            data_util.save_predictions(os.path.join(args.save, 'iter%d-epoch%d' % (iter, epoch), 'val'), sample['name'], inputs, target_for_sdf.numpy(), [x.numpy() for x in target_for_occs], vis_pred_sdf, pred_occs, sample['world2grid'], args.vis_dfs, args.truncation)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        mindspore.save_checkpoint(model, os.path.join(args.save, 'model-epoch%s-net.ckpt' % epoch))
        mindspore.save_checkpoint(optimizer, os.path.join(args.save, 'model-epoch%s-opt.ckpt' % epoch))
    log_file.close()
    if has_val:
        log_file_val.close()
