...
class DenseLayer(nn.SequentialCell):
    def __init__(self, in_channels, growth_rate):
        ...
        self.relu = nn.ReLU()
        self.conv = nn.Conv2d(in_channels, growth_rate, kernel_size=3, stride=1,
...
class TransitionDown(nn.SequentialCell):
    def __init__(self, in_channels):
        super().__init__()
        self.norm = nn.BatchNorm2d(num_features=in_channels, momentum=0.1)
        self.relu = nn.ReLU()
...
class TransitionUp(nn.Cell):
    def __init__(self, in_channels, out_channels):
        ...
        self.convTrans = nn.Conv2dTranspose(
            ...
            kernel_size=3, stride=2, padding=0, has_bias=True)

    def construct(self, x, skip):
        out = self.convTrans(x)
        out = center_crop(out, skip.shape[2], skip.shape[3])
        out = mindspore.ops.cat([out, skip], 1)
        return out
